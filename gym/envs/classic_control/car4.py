...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        u = action*[self.force_mag, self.force_mag]
        ...
        x = state_[0]
        y = state_[1]
        psi = state_[2] = angle_normalize(state_[2])
        # Vbdot = state_dot[0]
        # psi_dot = state_dot[1]
        # Vb_g = state_[2]
        ...
        # if (np.abs(x) < 1.) and (np.abs(psi) < np.deg2rad(2)):  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
        #     costs -= 2000
        #     if self.isLearning:
        #         done = True

        if np.abs(x) > 50.:# and np.abs(psi) < 0.1:  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
            costs += 1000
            if self.isLearning:
                done = True

        if np.abs(y) > 50.:# and np.abs(psi) < 0.1:  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
            costs += 1000
            if self.isLearning:
                done = True
    ...
    def reset(self):
        ...
        self.state[1] = self.np_random.uniform(low=-10., high=10., size=(1,))
        self.state[2] = self.np_random.uniform(low=-np.pi/6, high=np.pi/6, size=(1,))
