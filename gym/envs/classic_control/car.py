...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        state_ = self.ode4(state, self.dt, u)
        ...
        costs += (np.rad2deg(psi-psi_g)) ** 2
        costs += 2.*(Vb-Vb_g)**2
        costs += 0.1 * throttle ** 2
        costs += 0.1 * steer**2# + .1 * x_dot**2 + 1*self.out_of_bound(x) # + x**2 + .001 * (force ** 2)
        ...
        if np.abs(Vb-Vb_g) < 1. and np.abs(psi-psi_g) < 0.1:  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
            costs -= 100
            if self.isLearning:
                done = True
    ...
    def reset(self):
        ...
        # self.state[0] = -2.0
        # self.state[1] = -0.01
        self.state[0] = self.np_random.uniform(low=0., high=10., size=(1,))
        self.state[1] = self.np_random.uniform(low=-np.pi / 6, high=np.pi / 6, size=(1,))
        self.state[2] = self.state[0] + self.np_random.uniform(low=-5., high=5., size=(1,))
        self.state[3] = self.state[1] + self.np_random.uniform(low=-np.pi / 6, high=np.pi / 6, size=(1,))
    ...
    def dynamics(self, x0, u):
        ...
        thetadot = x0[0] * (u[1]) / self.L
        xdot = np.array([Vbdot, thetadot, 0., 0.])
        return xdot
