...
class CarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # costs += 0.01 * steer**2# + .1 * x_dot**2 + 1*self.out_of_bound(x) # + x**2 + .001 * (force ** 2)
        ...
        # if np.abs(Vb) < 0.2:# and np.abs(psi) < 0.1:  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
        #     costs -= 100
        #     if self.isLearning:
        #         done = True

        if np.abs(psi) < np.deg2rad(1):# and np.abs(psi) < 0.1:  # np.abs(Vb)<1. and  (np.array_equal([x, y], self.goal)):
            costs -= 100
            if self.isLearning:
                done = True
    ...
    def reset(self):
        ...
        # self.state[0] = -2.0
        # self.state[1] = -0.01
        self.state[0] = 0#self.np_random.uniform(low=-10., high=10., size=(1,))
